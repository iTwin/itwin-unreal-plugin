...
//! Describes the second "feature ID" attribute of a primitive (named _FEATURE_ID_1)
template<class _T> struct FeatureIds_1
{
	using ComponentTypes = boost::mpl::set<float>;
	using Types = boost::mpl::set<Types::Scalar>;
	_T featureIds_1_;
...
public:
	...
	CesiumGltf::Model Tune()
	{
		...
		const auto findPropertyTable = [&](std::optional<UInt64AccessorView>& propertyTableView,
										   std::string_view const classProperty,
										   std::string const& propertyName) -> int64_t
		{
			...
			if (!structuralMetadataExtension)
				return -1;
			for (size_t propertyTableIndex = 0; propertyTableIndex < structuralMetadataExtension->propertyTables.size(); ++propertyTableIndex)
			{
		...
		const int64_t elementPropertyTableIndex = findPropertyTable(elementPropertyTableView_, "MeshPart", "element");

		// the Mesh-Export service will put material IDs in a second table
		const int64_t materialPropertyTableIndex = findPropertyTable(materialPropertyTableView_, "MeshPart1", "material");
		...
		for (const auto& mesh: model_.meshes)
		{
			...
			for (const auto& primitive: mesh.primitives)
			{
				...
				ProcessPrimitive(std::make_tuple(
					...
					AccessorViews::Maker<AccessorViews::FeatureIds_1>{materialFeatIdsAccessorIndex},
		...
		const auto processPiece = [&](const auto& indexIndices)
			{
				...
				// Service (should be the case in 07/2024)
				std::optional<uint64_t> itwinMatID;
				if (groupIt == rules_.elementToGroup_.end())
				{
					if (accessorViews.featureIds_1_.status() == CesiumGltf::AccessorViewStatus::Valid)
						itwinMatID = (*materialPropertyTableView_)[accessorViews.featureIds_1_[accessorViews.indices_[indexIndices[0]][0]][0]];
				}
				else
				{
...
void GltfTuner::ParseTilesetJson(const rapidjson::Document& tilesetJson)
{
		...
		++matIt)
	{
		...
		matInfo.name = nameIt->value.GetString();
	}
	{
		std::scoped_lock lock(impl_->mutex_);
