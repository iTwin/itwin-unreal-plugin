...
#               PLUGIN_COVERED_MODULES # Specifies which modules to select for code coverage. If omitted, all modules will be selected.
#                   MyPlugin1_ModuleA
#                   MyPlugin1_ModuleB
#           PLUGIN MyPlugin2
	...
	function (setupDirs pluginName projectDependenciesRef addedFilesRef extraTestsRef)
		...
		if (DEFINED funcArgs_SOURCE)
			...
			foreach (sourceItem ${sourceItems})
				...
				createSymlink ("${realSrcAbsDir}/Source/${sourceItem}" "${projectAbsDir}${srcDirRel}/Source/${sourceItem}" addedFiles_local)
			endforeach ()
			...
			foreach (item ${otherItems})
				createSymlink ("${realSrcAbsDir}/${item}" "${projectAbsDir}${srcDirRel}/${item}" addedFiles_local)
			endforeach ()
		endif ()
		...
		if (DEFINED funcArgs_DEPENDS)
			...
			endforeach ()
			# Create extra directory symlinks (BeBuildConfig/ used for options, and GeneratedShaderTest/ for GetPerFeatureValues.ush unit test)
		...
		if ("${pluginName}" STREQUAL ITwinForUnreal)
			...
			endforeach ()
			# Source code of the (modified) CesiumRuntime and the (unmodified) CesiumEditor are symlinked as well:
			foreach (item Source/CesiumRuntime Source/CesiumEditor)
...
,\"defines\":${definesJson}\
	...
	set (extraTests "")
	# We have to ensure that all obsolete files/symlinks created during the previous cmake run
	...
	foreach (plugin ${plugins})
		...
		cmake_parse_arguments (
			...
			"PLUGIN_DEPENDS;PLUGIN_COVERED_MODULES"
			${plugin}
		)
		set (sourceArgs "")
		...
		list (APPEND pluginNames "${pluginArgs_PLUGIN}")
		set (pluginCoveredModules_${pluginArgs_PLUGIN} ${pluginArgs_PLUGIN_COVERED_MODULES})
	endforeach ()
	...
	foreach (addedFile_old ${addedFiles_old})
		...
		if (IS_SYMLINK "${addedFile_old}")
			...
			# and is no longer added by this script, then should we delete it?
	...
	#set var paths
	if( WIN32)
		# Use xxx-Cmd.exe variant of UE, easier to get stdout/stderr, especially with OpenCppCoverage.
		set (UnrealLaunchPath "${BE_UNREAL_ENGINE_DIR}/Binaries/Win64/UnrealEditor$<$<CONFIG:Debug>:-Win64-Debug>$<$<CONFIG:UnrealDebug>:-Win64-DebugGame>-Cmd.exe")
	elseif (APPLE)
	...
	if( WIN32)
		...
		set(ubtGeneratedProjectFilesDir "${projectAbsDir}/Intermediate/ProjectFiles")
		# With UE from source, project files are not generated in the project folder...
		if (NOT EXISTS "${ubtGeneratedProjectFilesDir}/${projectName}.vcxproj")
			set(ubtGeneratedProjectFilesDir "${BE_UNREAL_ENGINE_DIR}/Intermediate/ProjectFiles")
			file (CREATE_LINK "${BE_UNREAL_ENGINE_DIR}/Intermediate/ProjectFiles/UECommon.props"
							  "${projectAbsDir}/Intermediate/ProjectFiles_be/UECommon.props" SYMBOLIC)
		endif()
		foreach (extension .vcxproj .vcxproj.user .vcxproj.filters)
			file (READ "${ubtGeneratedProjectFilesDir}/${projectName}${extension}" vcxprojContent)
		...
		if (BE_USE_OFFICIAL_UNREAL) # no Debug_Editor config there!
			set(_mapCfgDebug DebugGame_Editor)
		else()
			set(_mapCfgDebug Debug_Editor)
		endif()
		set_target_properties(${projectName}_Editor PROPERTIES
			MAP_IMPORTED_CONFIG_DEBUG ${_mapCfgDebug}
		...
		add_custom_target (${projectName}_Editor_NoUnity ALL
			COMMAND "${BE_UNREAL_ENGINE_DIR}/Build/BatchFiles/Build.bat" ${projectName}Editor Win64 $<$<CONFIG:Debug>:Debug>$<$<CONFIG:UnrealDebug>:DebugGame>$<$<CONFIG:Release>:Development> -Project="${projectAbsDir}/${projectName}.uproject" -WaitMutex -FromMsBuild -beNoUnity
		)
		...
		add_custom_target (${projectName}_Game ALL
			COMMAND "${BE_UNREAL_ENGINE_DIR}/Build/BatchFiles/Build.bat" ${projectName} Win64 $<$<CONFIG:Debug>:Debug>$<$<CONFIG:UnrealDebug>:DebugGame>$<$<CONFIG:Release>:Development> -Project="${projectAbsDir}/${projectName}.uproject" -WaitMutex -FromMsBuild "$<$<BOOL:${BE_COMFY}>:-beComfy>"
		)
		set_property (TARGET ${projectName}_Game PROPERTY VS_DEBUGGER_COMMAND "${projectAbsDir}/Binaries/Win64/${projectName}$<$<CONFIG:Debug>:-Win64-Debug>$<$<CONFIG:UnrealDebug>:-Win64-DebugGame>.exe")
		...
		if (NOT FOR_VERACODE)
			...
			add_custom_target (${projectName}_Game_Packaged ALL
				...
				COMMAND "${Python3_EXECUTABLE}"
						...
						-clientconfig=$<$<CONFIG:Debug>:Debug>$<$<CONFIG:UnrealDebug>:DebugGame>$<$<CONFIG:Release>:Development>
	...
	elseif (APPLE)
		...
		if (CMAKE_OSX_ARCHITECTURES)
			add_custom_target( ${projectName}_Game
				COMMAND "xcodebuild" "-quiet" "build" "-scheme" "${projectName}" "-configuration" "$<$<CONFIG:Debug>:Debug>$<$<CONFIG:UnrealDebug>:DebugGame>$<$<CONFIG:Release>:Development>" "-project" "${projectAbsDir}/Intermediate/ProjectFiles/${projectName} (Mac).xcodeproj" "-arch" "${CMAKE_OSX_ARCHITECTURES}"
				VERBATIM
			)
		else()
			add_custom_target( ${projectName}_Game
				COMMAND "xcodebuild" "-quiet" "build" "-scheme" "${projectName}" "-configuration" "$<$<CONFIG:Debug>:Debug>$<$<CONFIG:UnrealDebug>:DebugGame>$<$<CONFIG:Release>:Development>" "-project" "${projectAbsDir}/Intermediate/ProjectFiles/${projectName} (Mac).xcodeproj"
				VERBATIM
			)
		endif()
		if (NOT FOR_VERACODE)
		...
		if (CMAKE_OSX_ARCHITECTURES)
			add_custom_target( ${projectName}_Editor 
				COMMAND "xcodebuild" "-quiet" "build" "-scheme" "${projectName}Editor" "-configuration" "$<$<CONFIG:Debug>:Debug>$<$<CONFIG:UnrealDebug>:DebugGame>$<$<CONFIG:Release>:Development>" "-project" "${projectAbsDir}/Intermediate/ProjectFiles/${projectName}Editor (Mac).xcodeproj" "-arch" "${CMAKE_OSX_ARCHITECTURES}"
				VERBATIM
			)
		else()
			add_custom_target( ${projectName}_Editor 
				COMMAND "xcodebuild" "-quiet" "build" "-scheme" "${projectName}Editor" "-configuration" "$<$<CONFIG:Debug>:Debug>$<$<CONFIG:UnrealDebug>:DebugGame>$<$<CONFIG:Release>:Development>" "-project" "${projectAbsDir}/Intermediate/ProjectFiles/${projectName}Editor (Mac).xcodeproj"
				VERBATIM
			)
		endif()
		if (NOT FOR_VERACODE)
	...
	if (NOT funcArgs_NO_TEST AND NOT FOR_VERACODE)
		...
		foreach (chainedFlag "" "Chained")
			set (runCommand "${UnrealLaunchPath}" "${projectAbsDir}/${projectName}.uproject" "-ExecCmds=\"Automation RunTests Bentley${extraTests}\;Quit\"" -unattended -nopause -editor -stdout -nosound -nullrhi)
			if (BE_CODE_COVERAGE)
				# OpenCppCoverage wants backslashes for source/module paths.
				string (REPLACE "/" "\\" projectAbsDirNative "${projectAbsDir}")
				set (modulesArgs --modules "${projectAbsDirNative}\\Binaries\\Win64\\UnrealEditor-*.dll")
				foreach (pluginName ${pluginNames})
					# If a list of covered modules is specified for this plugin, then use it.
					# Otherwise, cover all modules for this plugin.
					if (pluginCoveredModules_${pluginName})
						foreach (module ${pluginCoveredModules_${pluginName}})
							list (APPEND modulesArgs --modules "${projectAbsDirNative}\\Plugins\\${pluginName}\\Binaries\\Win64\\UnrealEditor-${module}*.dll")
						endforeach ()
					else ()
						list (APPEND modulesArgs --modules "${projectAbsDirNative}\\Plugins\\${pluginName}\\Binaries\\Win64\\UnrealEditor-*.dll")
					endif ()
				endforeach ()
				set (runCommand "${BE_COVERAGE_TOOL_EXE}"
					--export_type "html:${CMAKE_BINARY_DIR}/CodeCoverage/$(Configuration)/${projectName}"
					--export_type "binary:${CMAKE_BINARY_DIR}/CodeCoverage/$(Configuration)/${projectName}/report.binary"
					--export_type "cobertura:${CMAKE_BINARY_DIR}/CodeCoverage/$(Configuration)/${projectName}/report.cobertura.xml"
					${modulesArgs}
			...
			add_custom_target (RunTests${chainedFlag}_${projectName} ALL # "ALL" so that target is enabled and MSBuild can build it.
				...
				# UnrealEditor-Win64-Debug.exe loads the Debug binaries (this config is mapped to CMake Debug config).
				# UnrealEditor-Win64-DebugGame.exe loads the DebugGame binaries (this config is mapped to CMake UnrealDebug config).
				...
				COMMAND "$<$<NOT:$<CONFIG:Debug,UnrealDebug,Release>>:TARGET_NOT_COMPATIBLE_WITH_THIS_CONFIG>"
	...
	if(WIN32)
		...
		if (NOT beIsUE5ProjectImported)
			if (EXISTS "${projectAbsDir}/Intermediate/ProjectFiles/UE5.vcxproj")
			...
			else() # building from source:
				include_external_msproject (UE5 "${BE_UNREAL_ENGINE_DIR}/Intermediate/ProjectFiles/UE5.vcxproj" PLATFORM x64)
				set_target_properties(UE5 PROPERTIES
					MAP_IMPORTED_CONFIG_DEBUG Debug_Editor
					MAP_IMPORTED_CONFIG_UNREALDEBUG DebugGame_Editor
					MAP_IMPORTED_CONFIG_RELEASE Development_Editor
				)
			endif()
