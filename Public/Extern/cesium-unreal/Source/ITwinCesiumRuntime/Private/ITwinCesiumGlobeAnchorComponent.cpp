...
UITwinCesiumGlobeAnchorComponent::GetGeoreference() const {
  return this->Georeference;
}

void UITwinCesiumGlobeAnchorComponent::SetGeoreference(
    TSoftObjectPtr<AITwinCesiumGeoreference> NewGeoreference) {
  AITwinCesiumGeoreference* pOriginal = this->ResolvedGeoreference;
...
UITwinCesiumGlobeAnchorComponent::GetResolvedGeoreference() const {
  return this->ResolvedGeoreference;
}

FVector
UITwinCesiumGlobeAnchorComponent::GetEarthCenteredEarthFixedPosition() const {
...
FMatrix
UITwinCesiumGlobeAnchorComponent::GetActorToEarthCenteredEarthFixedMatrix() const {
  if (!this->_actorToECEFIsValid) {
    const_cast<UITwinCesiumGlobeAnchorComponent*>(this)
...
void UITwinCesiumGlobeAnchorComponent::SetActorToEarthCenteredEarthFixedMatrix(
    const FMatrix& Value) {
...
bool UITwinCesiumGlobeAnchorComponent::GetTeleportWhenUpdatingTransform() const {
  return this->TeleportWhenUpdatingTransform;
}

void UITwinCesiumGlobeAnchorComponent::SetTeleportWhenUpdatingTransform(bool Value) {
  this->TeleportWhenUpdatingTransform = Value;
}

bool UITwinCesiumGlobeAnchorComponent::GetAdjustOrientationForGlobeWhenMoving()
    const {
  return this->AdjustOrientationForGlobeWhenMoving;
}

void UITwinCesiumGlobeAnchorComponent::SetAdjustOrientationForGlobeWhenMoving(
    bool Value) {
  this->AdjustOrientationForGlobeWhenMoving = Value;
}

void UITwinCesiumGlobeAnchorComponent::MoveToEarthCenteredEarthFixedPosition(
...
void UITwinCesiumGlobeAnchorComponent::SnapLocalUpToEllipsoidNormal() {
  ...
  FVector ellipsoidNormal = UITwinCesiumWgs84Ellipsoid::GeodeticSurfaceNormal(
...
void UITwinCesiumGlobeAnchorComponent::SnapToEastSouthUp() {
...
void UITwinCesiumGlobeAnchorComponent::Sync() {
  ...
  if (Previous != Next) {
    ...
    if (this->ResolvedGeoreference) {
      this->ResolvedGeoreference->OnGeoreferenceUpdated.AddUniqueDynamic(
          this,
          &UITwinCesiumGlobeAnchorComponent::_onGeoreferenceChanged);
...
void UITwinCesiumGlobeAnchorComponent::InvalidateResolvedGeoreference() {
  // This method is deprecated and no longer does anything.
}

FVector UITwinCesiumGlobeAnchorComponent::GetLongitudeLatitudeHeight() const {
  return UITwinCesiumWgs84Ellipsoid::
      EarthCenteredEarthFixedToLongitudeLatitudeHeight(
          this->GetEarthCenteredEarthFixedPosition());
}

void UITwinCesiumGlobeAnchorComponent::MoveToLongitudeLatitudeHeight(
...
FQuat UITwinCesiumGlobeAnchorComponent::GetEastSouthUpRotation() const {
  if (!this->_actorToECEFIsValid) {
    // Only log a warning if we're actually in a world. Otherwise we'll spam the
    // log when editing a CDO.
    if (this->GetWorld()) {
...
FQuat UITwinCesiumGlobeAnchorComponent::GetEarthCenteredEarthFixedRotation() const {
  if (!this->_actorToECEFIsValid) {
    // Only log a warning if we're actually in a world. Otherwise we'll spam the
    // log when editing a CDO.
    if (this->GetWorld()) {
...
void UITwinCesiumGlobeAnchorComponent::Serialize(FArchive& Ar) {
  ...
  if (CesiumVersion <
      FITwinCesiumCustomVersion::GlobeAnchorTransformationAsFMatrix) {
    memcpy(
...
void UITwinCesiumGlobeAnchorComponent::OnComponentCreated() {
  Super::OnComponentCreated();
  this->_actorToECEFIsValid = false;
}

#if WITH_EDITOR
void UITwinCesiumGlobeAnchorComponent::PostEditChangeProperty(
    FPropertyChangedEvent& PropertyChangedEvent) {
  if (PropertyChangedEvent.Property) {
    FName propertyName = PropertyChangedEvent.Property->GetFName();

    if (propertyName ==
        GET_MEMBER_NAME_CHECKED(UITwinCesiumGlobeAnchorComponent, Georeference)) {
...
void UITwinCesiumGlobeAnchorComponent::OnRegister() {
  Super::OnRegister();
  ...
  if (pOwnerRoot) {
    pOwnerRoot->TransformUpdated.AddUObject(
        this,
        &UITwinCesiumGlobeAnchorComponent::_onActorTransformChanged);
  }

  this->ResolveGeoreference();
}

void UITwinCesiumGlobeAnchorComponent::OnUnregister() {
  Super::OnUnregister();
...
CesiumGeospatial::GlobeAnchor
UITwinCesiumGlobeAnchorComponent::_createNativeGlobeAnchor() const {
  return createNativeGlobeAnchor(this->ActorToEarthCenteredEarthFixedMatrix);
}

USceneComponent*
UITwinCesiumGlobeAnchorComponent::_getRootComponent(bool warnIfNull) const {
  const AActor* pOwner = this->GetOwner();
  if (!IsValid(pOwner)) {
    if (warnIfNull) {
      UE_LOG(
...
FTransform UITwinCesiumGlobeAnchorComponent::_getCurrentRelativeTransform() const {
  const USceneComponent* pOwnerRoot = this->_getRootComponent(true);
  return pOwnerRoot->GetRelativeTransform();
}

void UITwinCesiumGlobeAnchorComponent::_setCurrentRelativeTransform(
...
CesiumGeospatial::GlobeAnchor UITwinCesiumGlobeAnchorComponent::
    _createOrUpdateNativeGlobeAnchorFromRelativeTransform(
        const FTransform& newRelativeTransform) {
  AITwinCesiumGeoreference* pGeoreference = this->ResolvedGeoreference;
...
CesiumGeospatial::GlobeAnchor
UITwinCesiumGlobeAnchorComponent::_createOrUpdateNativeGlobeAnchorFromECEF(
...
void UITwinCesiumGlobeAnchorComponent::_onActorTransformChanged(
...
void UITwinCesiumGlobeAnchorComponent::_setNewActorToECEFFromRelativeTransform() {
  // This method is equivalent to
...
void UITwinCesiumGlobeAnchorComponent::_onGeoreferenceChanged() {
  if (this->_actorToECEFIsValid) {
    this->SetActorToEarthCenteredEarthFixedMatrix(
        this->ActorToEarthCenteredEarthFixedMatrix);
  }
