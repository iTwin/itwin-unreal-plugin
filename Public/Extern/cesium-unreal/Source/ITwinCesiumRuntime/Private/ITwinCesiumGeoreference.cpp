...
AITwinCesiumGeoreference* FindGeoreferenceAncestor(AActor* Actor) {
  AActor* Current = Actor;

  while (IsValid(Current)) {
    AITwinCesiumGeoreference* Georeference = Cast<AITwinCesiumGeoreference>(Current);
...
FindGeoreferenceWithTag(const UObject* WorldContextObject, const FName& Tag) {
  ...
  AITwinCesiumGeoreference* Georeference = nullptr;
  ...
  for (TActorIterator<AActor> actorIterator(
           World,
           AITwinCesiumGeoreference::StaticClass(),
           flags);
       actorIterator;
       ++actorIterator) {
    ...
    if (actor->GetLevel() == World->PersistentLevel &&
      ...
      Georeference = Cast<AITwinCesiumGeoreference>(actor);
      break;
    }
  }
...
FindGeoreferenceWithDefaultName(const UObject* WorldContextObject) {
  ...
  AITwinCesiumGeoreference* Candidate = FindObject<AITwinCesiumGeoreference>(
...
CreateDefaultGeoreference(const UObject* WorldContextObject, const FName& Tag) {
  ...
  AITwinCesiumGeoreference* Georeference =
      World->SpawnActor<AITwinCesiumGeoreference>(spawnParameters);
  if (Georeference) {
...
/*static*/ const double AITwinCesiumGeoreference::kMinimumScale = 1.0e-6;

/*static*/ AITwinCesiumGeoreference*
AITwinCesiumGeoreference::GetDefaultGeoreference(const UObject* WorldContextObject) {
...
/*static*/ AITwinCesiumGeoreference*
AITwinCesiumGeoreference::GetDefaultGeoreferenceForActor(AActor* Actor) {
  if (!IsValid(Actor))
    return nullptr;

  AITwinCesiumGeoreference* Georeference = FindGeoreferenceAncestor(Actor);
  if (IsValid(Georeference))
    return Georeference;

  return AITwinCesiumGeoreference::GetDefaultGeoreference(Actor);
}

FVector AITwinCesiumGeoreference::GetOriginLongitudeLatitudeHeight() const {
  return FVector(OriginLongitude, OriginLatitude, OriginHeight);
}

void AITwinCesiumGeoreference::SetOriginLongitudeLatitudeHeight(
...
FVector AITwinCesiumGeoreference::GetOriginEarthCenteredEarthFixed() const {
  return UITwinCesiumWgs84Ellipsoid::
      LongitudeLatitudeHeightToEarthCenteredEarthFixed(
          this->GetOriginLongitudeLatitudeHeight());
}

void AITwinCesiumGeoreference::SetOriginEarthCenteredEarthFixed(
...
double AITwinCesiumGeoreference::GetOriginLatitude() const {
  return this->OriginLatitude;
}

void AITwinCesiumGeoreference::SetOriginLatitude(double NewValue) {
  this->OriginLatitude = NewValue;
  this->UpdateGeoreference();
}

double AITwinCesiumGeoreference::GetOriginLongitude() const {
  return this->OriginLongitude;
}

void AITwinCesiumGeoreference::SetOriginLongitude(double NewValue) {
  this->OriginLongitude = NewValue;
  this->UpdateGeoreference();
}

double AITwinCesiumGeoreference::GetOriginHeight() const {
  return this->OriginHeight;
}

void AITwinCesiumGeoreference::SetOriginHeight(double NewValue) {
  this->OriginHeight = NewValue;
  this->UpdateGeoreference();
}

double AITwinCesiumGeoreference::GetScale() const { return this->Scale; }

void AITwinCesiumGeoreference::SetScale(double NewValue) {
  if (NewValue < AITwinCesiumGeoreference::kMinimumScale) {
    this->Scale = AITwinCesiumGeoreference::kMinimumScale;
  } else {
    this->Scale = NewValue;
  }
  this->UpdateGeoreference();
...
APlayerCameraManager* AITwinCesiumGeoreference::GetSubLevelCamera() const {
  return this->SubLevelCamera_DEPRECATED;
}

void AITwinCesiumGeoreference::SetSubLevelCamera(APlayerCameraManager* NewValue) {
  this->SubLevelCamera_DEPRECATED = NewValue;
}

#if WITH_EDITOR
bool AITwinCesiumGeoreference::GetShowLoadRadii() const {
  return this->ShowLoadRadii;
}

void AITwinCesiumGeoreference::SetShowLoadRadii(bool NewValue) {
  this->ShowLoadRadii = NewValue;
}
#endif // WITH_EDITOR

FVector AITwinCesiumGeoreference::TransformLongitudeLatitudeHeightPositionToUnreal(
...
FVector AITwinCesiumGeoreference::TransformUnrealPositionToLongitudeLatitudeHeight(
...
FRotator AITwinCesiumGeoreference::TransformEastSouthUpRotatorToUnreal(
...
FMatrix AITwinCesiumGeoreference::ComputeEastSouthUpToUnrealTransformation(
    const FVector& UnrealLocation) const {
  FVector ecef =
...
FMatrix AITwinCesiumGeoreference::
    ComputeEastSouthUpAtEarthCenteredEarthFixedPositionToUnrealTransformation(
        const FVector& EarthCenteredEarthFixedPosition) const {
...
FMatrix AITwinCesiumGeoreference::ComputeUnrealToEastSouthUpTransformation(
    const FVector& UnrealLocation) const {
...
#if WITH_EDITOR
void AITwinCesiumGeoreference::PlaceGeoreferenceOriginHere() {
...
void AITwinCesiumGeoreference::CreateSubLevelHere() {
  ...
  UITwinCesiumSubLevelComponent* LevelComponent =
      Cast<UITwinCesiumSubLevelComponent>(LevelInstance->AddComponentByClass(
          UITwinCesiumSubLevelComponent::StaticClass(),
          false,
          FTransform::Identity,
          false));
...
void AITwinCesiumGeoreference::_showSubLevelLoadRadii() const {
  ...
  for (const auto& pLevelWeak :
    ...
    UITwinCesiumSubLevelComponent* pComponent =
        pLevel->FindComponentByClass<UITwinCesiumSubLevelComponent>();
...
bool AITwinCesiumGeoreference::ShouldTickIfViewportsOnly() const { return true; }

void AITwinCesiumGeoreference::Tick(float DeltaTime) {
...
void AITwinCesiumGeoreference::Serialize(FArchive& Ar) {
  Super::Serialize(Ar);
...
void AITwinCesiumGeoreference::BeginPlay() {
  Super::BeginPlay();
...
void AITwinCesiumGeoreference::OnConstruction(const FTransform& Transform) {
...
void AITwinCesiumGeoreference::PostLoad() {
  Super::PostLoad();
  ...
  if (CesiumVersion < FITwinCesiumCustomVersion::OriginShiftComponent &&
    ...
    if (SubLevelActor) {
      ...
      if (SubLevelActor->FindComponentByClass<UITwinCesiumOriginShiftComponent>() ==
          nullptr) {

        UITwinCesiumOriginShiftComponent* OriginShift =
            Cast<UITwinCesiumOriginShiftComponent>(
...
#if WITH_EDITOR
void AITwinCesiumGeoreference::PostEditChangeProperty(FPropertyChangedEvent& event) {
  ...
  CESIUM_POST_EDIT_CHANGE(propertyName, AITwinCesiumGeoreference, OriginPlacement);
  CESIUM_POST_EDIT_CHANGE(propertyName, AITwinCesiumGeoreference, OriginLongitude);
  CESIUM_POST_EDIT_CHANGE(propertyName, AITwinCesiumGeoreference, OriginLatitude);
  CESIUM_POST_EDIT_CHANGE(propertyName, AITwinCesiumGeoreference, OriginHeight);
  CESIUM_POST_EDIT_CHANGE(propertyName, AITwinCesiumGeoreference, Scale);
}
...
void AITwinCesiumGeoreference::_createSubLevelsFromWorldComposition() {
  UWorld* pWorld = this->GetWorld();
  if (!IsValid(pWorld)) {
  ...
  for (const FWorldCompositionTile& level : allLevels) {
    ...
    FITwinCesiumSubLevel* pFound = this->CesiumSubLevels_DEPRECATED.FindByPredicate(
        [&levelName](FITwinCesiumSubLevel& subLevel) {
    ...
    UITwinCesiumSubLevelComponent* pLevelComponent =
        Cast<UITwinCesiumSubLevelComponent>(pLevelInstance->AddComponentByClass(
            UITwinCesiumSubLevelComponent::StaticClass(),
            false,
            FTransform::Identity,
            false));
...
FVector AITwinCesiumGeoreference::TransformLongitudeLatitudeHeightToEcef(
    const FVector& longitudeLatitudeHeight) const {
  return UITwinCesiumWgs84Ellipsoid::
      LongitudeLatitudeHeightToEarthCenteredEarthFixed(longitudeLatitudeHeight);
}

FVector AITwinCesiumGeoreference::TransformEcefToLongitudeLatitudeHeight(
...
FMatrix
AITwinCesiumGeoreference::ComputeEastNorthUpToEcef(const FVector& ecef) const {
  return UITwinCesiumWgs84Ellipsoid::EastNorthUpToEarthCenteredEarthFixed(ecef);
}

AITwinCesiumGeoreference::AITwinCesiumGeoreference() : AActor() {
  ...
  this->SubLevelSwitcher =
      CreateDefaultSubobject<UITwinCesiumSubLevelSwitcherComponent>(
          "SubLevelSwitcher");
}

void AITwinCesiumGeoreference::UpdateGeoreference() {
  this->_updateCoordinateSystem();

  // If we're in a sub-level, update its origin as well.
  UITwinCesiumSubLevelSwitcherComponent* pSwitcher = this->SubLevelSwitcher;
  if (IsValid(pSwitcher) && pSwitcher->GetTargetSubLevel() != nullptr) {
    if (pSwitcher->GetTargetSubLevel() == pSwitcher->GetCurrentSubLevel() ||
      ...
      UITwinCesiumSubLevelComponent* pComponent =
          pTarget->FindComponentByClass<UITwinCesiumSubLevelComponent>();
...
ITwinGeoTransforms AITwinCesiumGeoreference::GetGeoTransforms() const noexcept {
  // Because ITwinGeoTransforms is deprecated, we only lazily update it.
  return ITwinGeoTransforms(
      Ellipsoid::WGS84,
      glm::dvec3(this->_coordinateSystem.getLocalToEcefTransformation()[3]),
      this->GetScale() / 100.0);
}

FName AITwinCesiumGeoreference::DEFAULT_GEOREFERENCE_TAG =
