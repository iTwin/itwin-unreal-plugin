...
public:
  void Construct(
      ...
      const FString& Text,
      const FString& Url,
      const UITwinCreditsDecorator* InDecorator) {
...
public:
  FScreenCreditsDecorator(
      ...
      UITwinCreditsDecorator* InDecorator)
...
private:
  UITwinCreditsDecorator* Decorator;
};

UITwinCreditsDecorator::UITwinCreditsDecorator(
    const FObjectInitializer& ObjectInitializer)
    : URichTextBlockDecorator(ObjectInitializer) {}

TSharedPtr<ITextDecorator>
UITwinCreditsDecorator::CreateDecorator(URichTextBlock* InOwner) {
  return MakeShareable(new FScreenCreditsDecorator(InOwner, this));
}

const FSlateBrush* UITwinCreditsDecorator::FindImageBrush(int32 id) {
...
UITwinScreenCreditsWidget::UITwinScreenCreditsWidget(
    const FObjectInitializer& ObjectInitializer)
...
void UITwinScreenCreditsWidget::OnPopupClicked() {
  _showPopup = !_showPopup;
  BackgroundBlur->SetVisibility(
      _showPopup ? ESlateVisibility::Visible : ESlateVisibility::Hidden);
}

void UITwinScreenCreditsWidget::NativeConstruct() {
  ...
  if (RichTextOnScreen) {
    ...
    _decoratorOnScreen =
        static_cast<UITwinCreditsDecorator*>(RichTextOnScreen->GetDecoratorByClass(
            UITwinCreditsDecorator::StaticClass()));
    if (_decoratorOnScreen)
    {
        _decoratorOnScreen->PopupClicked.BindUObject(
            this,
            &UITwinScreenCreditsWidget::OnPopupClicked);
  ...
  if (RichTextPopup) {
    ...
    RichTextPopup->SetDefaultColorAndOpacity(
        FSlateColor(FLinearColor(1.f, 1.f, 1.f, 1.f)));
    _decoratorPopup = static_cast<UITwinCreditsDecorator*>(
        RichTextPopup->GetDecoratorByClass(UITwinCreditsDecorator::StaticClass()));
    if (_decoratorPopup)
    {
...
void UITwinScreenCreditsWidget::HandleImageRequest(
...
std::string UITwinScreenCreditsWidget::LoadImage(const std::string& url) {
  ...
  if (url.rfind(base64Prefix, 0) == 0) {
    ...
    HttpRequest->OnProcessRequestComplete().BindUObject(
        this,
        &UITwinScreenCreditsWidget::HandleImageRequest,
        _creditImages.Num() - 1);
...
void UITwinScreenCreditsWidget::SetCredits(
