...
// Sets default values
AITwinCesiumSunSky::AITwinCesiumSunSky() : AActor() {
...
void AITwinCesiumSunSky::_handleTransformUpdated(
...
void AITwinCesiumSunSky::OnConstruction(const FTransform& Transform) {
...
void AITwinCesiumSunSky::_spawnSkySphere() {
  ...
  UITwinCesiumGlobeAnchorComponent* GlobeAnchorComponent =
      NewObject<UITwinCesiumGlobeAnchorComponent>(
          SkySphereActor,
          TEXT("GlobeAnchor"));
...
double AITwinCesiumSunSky::_computeScale() const {
...
void AITwinCesiumSunSky::UpdateSkySphere() {
...
void AITwinCesiumSunSky::BeginPlay() {
  Super::BeginPlay();
  ...
  this->_transformUpdatedSubscription =
      this->RootComponent->TransformUpdated.AddUObject(
          this,
          &AITwinCesiumSunSky::_handleTransformUpdated);
...
void AITwinCesiumSunSky::EndPlay(const EEndPlayReason::Type EndPlayReason) {
...
void AITwinCesiumSunSky::Serialize(FArchive& Ar) {
  Super::Serialize(Ar);
  ...
  if (Ar.IsLoading() &&
      CesiumVersion < FITwinCesiumCustomVersion::GeoreferenceRefactoring) {
...
void AITwinCesiumSunSky::Tick(float DeltaSeconds) {
...
void AITwinCesiumSunSky::PostLoad() {
  Super::PostLoad();
...
bool AITwinCesiumSunSky::ShouldTickIfViewportsOnly() const { return true; }

void AITwinCesiumSunSky::_setSkyAtmosphereVisibility(bool bVisible) {
  if (IsValid(SkyLight)) {
...
void AITwinCesiumSunSky::_setSkySphereDirectionalLight() {
...
#if WITH_EDITOR
void AITwinCesiumSunSky::PostEditChangeProperty(
    FPropertyChangedEvent& PropertyChangedEvent) {

  const FName propertyName = (PropertyChangedEvent.Property)
                                 ? PropertyChangedEvent.Property->GetFName()
                                 : NAME_None;
  if (propertyName == GET_MEMBER_NAME_CHECKED(AITwinCesiumSunSky, SkySphereClass)) {
  ...
  if (propertyName ==
      GET_MEMBER_NAME_CHECKED(AITwinCesiumSunSky, UseMobileRendering)) {
  ...
  if (propertyName ==
          GET_MEMBER_NAME_CHECKED(AITwinCesiumSunSky, UseLevelDirectionalLight) ||
      propertyName ==
          GET_MEMBER_NAME_CHECKED(AITwinCesiumSunSky, LevelDirectionalLight)) {
    _setSkySphereDirectionalLight();
...
void AITwinCesiumSunSky::UpdateSun_Implementation() {
...
void AITwinCesiumSunSky::UpdateAtmosphereRadius() {
...
void AITwinCesiumSunSky::GetHMSFromSolarTime(
    double InSolarTime,
    int32& Hour,
    int32& Minute,
    int32& Second) {
...
void AITwinCesiumSunSky::SetSkyAtmosphereGroundRadius(
