...
BEGIN_DEFINE_SPEC(
    FITwinCesiumGlobeAnchorSpec,
    "Cesium.Unit.GlobeAnchor",
    EAutomationTestFlags::ApplicationContextMask |
        EAutomationTestFlags::ProductFilter)

TObjectPtr<AActor> pActor;
TObjectPtr<UITwinCesiumGlobeAnchorComponent> pGlobeAnchor;

END_DEFINE_SPEC(FITwinCesiumGlobeAnchorSpec)
...
void FITwinCesiumGlobeAnchorSpec::Define() {
  BeforeEach([this]() {
    ...
    AITwinCesiumGeoreference* pGeoreference =
        AITwinCesiumGeoreference::GetDefaultGeoreferenceForActor(pActor);
    pGeoreference->SetOriginLongitudeLatitudeHeight(FVector(1.0, 2.0, 3.0));

    this->pGlobeAnchor =
        Cast<UITwinCesiumGlobeAnchorComponent>(pActor->AddComponentByClass(
  ...
  It("maintains globe position when switching to a new georeference", [this]() {
    ...
    AITwinCesiumGeoreference* pNewGeoref = pWorld->SpawnActor<AITwinCesiumGeoreference>();
  ...
  It("gains correct orientation on call to SnapToEastSouthUp", [this]() {
    ...
    FMatrix enuToEcef =
        UITwinCesiumWgs84Ellipsoid::EastNorthUpToEarthCenteredEarthFixed(
  ...
  It("gains correct orientation on call to SnapLocalUpToEllipsoidNormal",
     [this]() {
       ...
       FVector surfaceNormal = UITwinCesiumWgs84Ellipsoid::GeodeticSurfaceNormal(
           this->pGlobeAnchor->GetEarthCenteredEarthFixedPosition());
