...
ITwinUnrealAssetAccessor::ITwinUnrealAssetAccessor()
...
CesiumAsync::Future<std::shared_ptr<CesiumAsync::IAssetRequest>>
ITwinUnrealAssetAccessor::request(
    const CesiumAsync::AsyncSystem& asyncSystem,
...
void ITwinUnrealAssetAccessor::tick() noexcept {
...
class FITwinCesiumReadFileWorker : public FNonAbandonableTask {
public:
  FITwinCesiumReadFileWorker(
      const std::string& url,
      const CesiumAsync::AsyncSystem& asyncSystem)
      : _url(url),
        _promise(
...
CesiumAsync::Future<std::shared_ptr<CesiumAsync::IAssetRequest>>
ITwinUnrealAssetAccessor::getFromFile(
    const CesiumAsync::AsyncSystem& asyncSystem,
    const std::string& url,
    const std::vector<CesiumAsync::IAssetAccessor::THeader>& headers) {
  check(!url.empty());
  ...
  auto pTaskOwner =
      std::make_unique<FAsyncTask<FITwinCesiumReadFileWorker>>(url, asyncSystem);

  FAsyncTask<FITwinCesiumReadFileWorker>* pTask = pTaskOwner.get();
