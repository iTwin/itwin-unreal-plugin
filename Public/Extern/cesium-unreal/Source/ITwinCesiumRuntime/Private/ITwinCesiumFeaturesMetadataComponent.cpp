...
using namespace ITwinCesiumEncodedFeaturesMetadata;

static const FString AutogeneratedMessage = "AUTOGENERATED DO NOT EDIT";

namespace {
void AutoFillPropertyTableDescriptions(
    TArray<FITwinCesiumPropertyTableDescription>& Descriptions,
  ...
  for (const auto& propertyTable : propertyTables) {
    ...
    FITwinCesiumPropertyTableDescription* pDescription =
        Descriptions.FindByPredicate(
            [&name = propertyTableName](
                const FITwinCesiumPropertyTableDescription& existingPropertyTable) {
    ...
    for (const auto& propertyIt : properties) {
      auto pExistingProperty = pDescription->Properties.FindByPredicate(
          [&propertyName = propertyIt.Key](
              const FITwinCesiumPropertyTablePropertyDescription& existingProperty) {
      ...
      if (pExistingProperty) {
        ...
        EITwinCesiumMetadataType type = pExistingProperty->PropertyDetails.Type;
        switch (type) {
        ...
        FITwinCesiumMetadataValue offset =
            UITwinCesiumPropertyTablePropertyBlueprintLibrary::GetOffset(
                propertyIt.Value);
        pExistingProperty->PropertyDetails.bHasOffset |=
            !UITwinCesiumMetadataValueBlueprintLibrary::IsEmpty(offset);

        FITwinCesiumMetadataValue scale =
            UITwinCesiumPropertyTablePropertyBlueprintLibrary::GetOffset(
                propertyIt.Value);
        pExistingProperty->PropertyDetails.bHasScale |=
            !UITwinCesiumMetadataValueBlueprintLibrary::IsEmpty(scale);

        continue;
      }

      FITwinCesiumPropertyTablePropertyDescription& property =
      ...
      property.PropertyDetails.ArraySize =
          UITwinCesiumPropertyTablePropertyBlueprintLibrary::GetArraySize(
              propertyIt.Value);
      property.PropertyDetails.bIsNormalized =
          UITwinCesiumPropertyTablePropertyBlueprintLibrary::IsNormalized(
              propertyIt.Value);
      ...
      property.PropertyDetails.bHasNoDataValue =
          !UITwinCesiumMetadataValueBlueprintLibrary::IsEmpty(noData);
      ...
      property.PropertyDetails.bHasDefaultValue =
          !UITwinCesiumMetadataValueBlueprintLibrary::IsEmpty(defaultValue);
...
void AutoFillPropertyTextureDescriptions(
    TArray<FITwinCesiumPropertyTextureDescription>& Descriptions,
    const FITwinCesiumModelMetadata& ModelMetadata) {
  ...
  for (const auto& propertyTexture : propertyTextures) {
    ...
    FITwinCesiumPropertyTextureDescription* pDescription =
        Descriptions.FindByPredicate(
            [&propertyTextureName =
    ...
    for (const auto& propertyIt : properties) {
      auto pExistingProperty = pDescription->Properties.FindByPredicate(
          [&propertyName =
               propertyIt.Key](const FITwinCesiumPropertyTexturePropertyDescription&
                                   existingProperty) {
      ...
      if (pExistingProperty) {
        ...
        FITwinCesiumMetadataValue offset =
            UITwinCesiumPropertyTexturePropertyBlueprintLibrary::GetOffset(
                propertyIt.Value);
        pExistingProperty->PropertyDetails.bHasOffset |=
            !UITwinCesiumMetadataValueBlueprintLibrary::IsEmpty(offset);
      ...
      FITwinCesiumPropertyTexturePropertyDescription& property =
          pDescription->Properties.Emplace_GetRef();
      property.Name = propertyIt.Key;
...
void AutoFillFeatureIdSetDescriptions(
    TArray<FITwinCesiumFeatureIdSetDescription>& Descriptions,
  ...
  for (const FITwinCesiumFeatureIdSet& featureIDSet : featureIDSets) {
    ...
    if (type == EITwinCesiumFeatureIdSetType::None || count == 0) {
    ...
    FITwinCesiumFeatureIdSetDescription* pDescription = Descriptions.FindByPredicate(
...
void UITwinCesiumFeaturesMetadataComponent::AutoFill() {
  const AITwinCesium3DTileset* pOwner = this->GetOwner<AITwinCesium3DTileset>();
  if (!pOwner) {
    return;
  }
  ...
  for (const UActorComponent* pComponent : pOwner->GetComponents()) {
    const UITwinCesiumGltfComponent* pGltf = Cast<UITwinCesiumGltfComponent>(pComponent);
    if (!pGltf) {
      continue;
    }

    const FITwinCesiumModelMetadata& modelMetadata = pGltf->Metadata;
    ...
    for (const USceneComponent* pChildComponent : childComponents) {
      const UITwinCesiumGltfPrimitiveComponent* pGltfPrimitive =
          Cast<UITwinCesiumGltfPrimitiveComponent>(pChildComponent);
      if (!pGltfPrimitive) {
        continue;
      }
...
GetOutputTypeForEncodedType(EITwinCesiumEncodedMetadataType Type) {
  switch (Type) {
...
FString GetSwizzleForEncodedType(EITwinCesiumEncodedMetadataType Type) {
  switch (Type) {
  case EITwinCesiumEncodedMetadataType::Scalar:
    return ".r";
  case EITwinCesiumEncodedMetadataType::Vec2:
...
bool IsSupportedPropertyTextureProperty(
    const FITwinCesiumMetadataPropertyDetails& PropertyDetails) {
  if (PropertyDetails.bIsArray &&
      PropertyDetails.Type != EITwinCesiumMetadataType::Scalar) {
...
UMaterialExpressionMaterialFunctionCall* GenerateNodesForFeatureIdTexture(
    const FITwinCesiumFeatureIdSetDescription& Description,
    TArray<UMaterialExpression*>& AutoGeneratedNodes,
...
UMaterialExpressionParameter* GenerateParameterNodeWithGivenType(
    const EITwinCesiumEncodedMetadataType Type,
    const FString& Name,
    TArray<UMaterialExpression*>& AutoGeneratedNodes,
    UMaterialFunctionMaterialLayer* TargetMaterialLayer,
    int32 NodeX,
  ...
  if (Type == EITwinCesiumEncodedMetadataType::Vec2 ||
...
void GenerateNodesForPropertyTable(
    const FITwinCesiumPropertyTableDescription& PropertyTable,
    TArray<UMaterialExpression*>& AutoGeneratedNodes,
    UMaterialFunctionMaterialLayer* TargetMaterialLayer,
    int32& NodeX,
    int32& NodeY,
  ...
  for (const FITwinCesiumPropertyTablePropertyDescription& Property :
       PropertyTable.Properties) {
    if (Property.EncodingDetails.Conversion ==
            EITwinCesiumEncodedMetadataConversion::None ||
...
void GenerateNodesForPropertyTexture(
    const FITwinCesiumPropertyTextureDescription& PropertyTexture,
    TArray<UMaterialExpression*>& AutoGeneratedNodes,
    UMaterialFunctionMaterialLayer* TargetMaterialLayer,
    int32& NodeX,
    int32& NodeY,
  ...
  for (const FITwinCesiumPropertyTexturePropertyDescription& Property :
...
void GenerateMaterialNodes(
    UITwinCesiumFeaturesMetadataComponent* pComponent,
    TArray<UMaterialExpression*>& AutoGeneratedNodes,
  ...
  for (const FITwinCesiumFeatureIdSetDescription& featureIdSet :
       pComponent->FeatureIdSets) {
    if (featureIdSet.Type == EITwinCesiumFeatureIdSetType::None) {
      continue;
    }

    UMaterialExpressionMaterialFunctionCall* GetFeatureIdCall = nullptr;
    if (featureIdSet.Type == EITwinCesiumFeatureIdSetType::Texture) {
    ...
    if (!featureIdSet.PropertyTableName.IsEmpty()) {
      const FITwinCesiumPropertyTableDescription* pPropertyTable =
          pComponent->PropertyTables.FindByPredicate(
              [&name = featureIdSet.PropertyTableName](
                  const FITwinCesiumPropertyTableDescription&
  ...
  // set.
  for (const FITwinCesiumPropertyTableDescription& propertyTable :
  ...
  for (const FString& propertyTextureName : pComponent->PropertyTextureNames) {
    const FITwinCesiumPropertyTextureDescription* pPropertyTexture =
  ...
  for (const FITwinCesiumPropertyTextureDescription& propertyTexture :
...
void UITwinCesiumFeaturesMetadataComponent::GenerateMaterial() {
  AITwinCesium3DTileset* pTileset = Cast<AITwinCesium3DTileset>(this->GetOwner());
  if (!pTileset) {
    return;
  }
