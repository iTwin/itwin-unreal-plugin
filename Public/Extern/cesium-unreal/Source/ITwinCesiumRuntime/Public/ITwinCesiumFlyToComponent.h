...
#include "ITwinCesiumFlyToComponent.generated.h"
 ...
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FITwinCesiumFlightCompleted);

/**
 * The delegate for when the Actor's flight is interrupted.
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FITwinCesiumFlightInterrupted);
...
class ITWINCESIUMRUNTIME_API UITwinCesiumFlyToComponent
    : public UITwinCesiumGlobeAnchoredActorComponent {
  GENERATED_BODY()

public:
  UITwinCesiumFlyToComponent();
  ...
  FITwinCesiumFlightCompleted OnFlightComplete;
   ...
   */
  UPROPERTY(BlueprintAssignable, Category = "Cesium");
  FITwinCesiumFlightInterrupted OnFlightInterrupted;
