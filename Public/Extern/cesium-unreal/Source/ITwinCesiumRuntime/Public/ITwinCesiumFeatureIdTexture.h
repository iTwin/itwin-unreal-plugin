...
#include "ITwinCesiumFeatureIdTexture.generated.h"
...
/**
 * @brief Reports the status of a FITwinCesiumFeatureIdTexture. If the feature ID
 ...
 * the corresponding {@link FITwinCesiumPropertyTable} to access per-pixel metadata.
 */
USTRUCT(BlueprintType)
struct ITWINCESIUMRUNTIME_API FITwinCesiumFeatureIdTexture {
  GENERATED_USTRUCT_BODY()

public:
  /**
   * @brief Constructs an empty feature ID texture instance. Empty feature ID
   * textures can be constructed while trying to convert a FITwinCesiumFeatureIdSet
...
private:
  ...
  friend class UITwinCesiumFeatureIdTextureBlueprintLibrary;
};

UCLASS()
class ITWINCESIUMRUNTIME_API UITwinCesiumFeatureIdTextureBlueprintLibrary
...
public:
  ...
  GetFeatureTableName(UPARAM(ref)
                          const FITwinCesiumFeatureIdTexture& FeatureIDTexture);
   ...
   * coordinates. The feature ID can be used with a FITwinCesiumPropertyTable to
   ...
   * feature ID can be used with a FITwinCesiumPropertyTable to retrieve the
   * per-vertex metadata.
   *
   ...
   * FITwinCesiumPropertyTable to retrieve the corresponding metadata.
   *
   * If the feature ID texture is invalid, this returns -1.
   */
  UFUNCTION(
