...
#include "ITwinCesiumPropertyArrayBlueprintLibrary.generated.h"
 ...
 */
UCLASS()
class ITWINCESIUMRUNTIME_API UITwinCesiumPropertyArrayBlueprintLibrary
...
public:
  ...
  GetElementValueType(UPARAM(ref) const FITwinCesiumPropertyArray& array);
  ...
  UFUNCTION(
      ...
      BlueprintPure,
      Category = "Cesium|Metadata|PropertyArray")
  static int64 GetArraySize(UPARAM(ref) const FITwinCesiumPropertyArray& Array);

  /**
   * Retrieves an element from the array as a FITwinCesiumMetadataValue. The value
   * can then be retrieved as a specific Blueprints type.
   *
   * If the index is out-of-bounds, this returns a bogus FITwinCesiumMetadataValue of
   * an unknown type.
   *
   * @param Index The index of the array element to retrieve.
   * @return The element as a FITwinCesiumMetadataValue.
   */
  UFUNCTION(
      BlueprintCallable,
      BlueprintPure,
  ...
  GetValue(UPARAM(ref) const FITwinCesiumPropertyArray& Array, int64 Index);
  ...
  static int64 GetSize(UPARAM(ref) const FITwinCesiumPropertyArray& Array);
