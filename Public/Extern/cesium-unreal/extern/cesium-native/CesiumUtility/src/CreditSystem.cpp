...
CreditSystem::getSnapshot(CreditFilteringMode filteringMode) noexcept {
  ...
  // sort credits, first on priority, then on their number of occurrences
  std::sort(
      currentCredits.begin(),
      currentCredits.end(),
      [&](const Credit& a, const Credit& b) {
        // compare custom priority before count/id, if any
        int32_t aPriority = this->_credits[a._id].priority;
        int32_t bPriority = this->_credits[b._id].priority;
