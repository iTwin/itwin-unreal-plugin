	...
	std::optional<uint32> const& UVIndexForFeatures) const
{
	...
	if (SrcMaterialInstance != nullptr)
	{
		...
		if (Options.bCreateNewMaterialInstance || Options.bPerElementColorationMode)
		{
			...
			ExtractedEntity.TextureFlags.InvalidateAll();
		}
	...
	FITwinMeshExtractionOptions const& InOptions /*= {}*/)
{
	...
	{
		auto* ElemInTile = SceneTile.FindElementFeaturesSLOW(EltID);
		if (!ensure(ElemInTile))
			continue;
		auto&& ExtractionEntry = SceneTile.ExtractedElementSLOW(*ElemInTile);
		if (ExtractionEntry.second) // was inserted, ie not extracted previously
		{
			// extract it now
			auto& ExtractedEntity =
				ExtractionEntry.first.get().Entities.emplace_back(FITwinExtractedEntity{ EltID });
			...
			{
				ExtractionEntry.first.get().Entities.pop_back();
			}
		}
	}
